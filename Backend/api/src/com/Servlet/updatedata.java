    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for(int i=0;i<list.size();i++){
            if(string==""){
                ...
                string3= "`"+(String)list.get(i)+"`='"+request.getParameter((String)list.get(i))+"'";
            }else{
                string=string+','+"`"+(String)list.get(i)+"` varchar(255) DEFAULT NULL";
                string2=string2+","+(String)list.get(i);
                string3=string3+","+"`"+(String)list.get(i)+"`='"+request.getParameter((String)list.get(i))+"'";
            }
        }
        string3=string3;
        request.setCharacterEncoding("utf-8");//设置编码方式，防止中文乱码

        String sql = "UPDATE  "+request.getParameter("table")+" set "+string3+" WHERE `id`='"+request.getParameter("id")+"'";
